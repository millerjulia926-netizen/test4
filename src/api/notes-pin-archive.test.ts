...
describe.runIf(dbAvailable)("notes pin and archive API", () => {
  ...
  it("pins a note and returns pinned notes first in the list", async () => {
    const { app, token } = await signupAndGetToken("alice@example.com");

    const older = await request(app)
      .post("/notes")
      .set("Authorization", `Bearer ${token}`)
      .send({ title: "Older note", content: "First" });

    const newer = await request(app)
      .post("/notes")
      .set("Authorization", `Bearer ${token}`)
      .send({ title: "Newer note", content: "Second" });

    const pinned = await request(app)
      .patch(`/notes/${older.body.id}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ isPinned: true });
    ...
    expect(listed.body[0].id).toBe(older.body.id);
    expect(listed.body[1].id).toBe(newer.body.id);
  });
